...
agent 完成一个任务后**不会退出**，终端继续提示"继续对话"，你可以接着提问或要求修改：

```text
任务 > buggy_wordcount.py 统计单词数有 bug，请修复并验证
（agent 读→改→跑→总结，输出 单词数: 11）
继续对话：输入下一个任务；空行或 退出/exit 结束。
任务 > 把程序改成从标准输入读取文本，不要再传文件路径
（agent 记住第一轮的工作，直接改造并重新验证）
任务 >                ← 空行结束会话
```

- 每个追问作为新的 `user` 消息追加进**同一份**对话历史，上下文跨轮保留（初始任务常驻、历史滑动裁剪，保证协议合法）。
- 这正体现了考核要求的"对话历史与上下文管理"：不仅管理单次任务内的工具轮次，也管理整个会话。
- 离线 `--mock` 演示一轮后自动退出。
...
44 项测试全部通过，覆盖：工具执行、超时杀进程、LLM 重试、主循环四场景、上下文裁剪协议合法性、工作区沙箱（越界拦截）、多轮对话上下文保留、REPL 行为、CLI 端到端。
...
- **多轮对话**：任务完成后不退出，追问追加进同一份历史、跨轮保留，模型可基于之前的完成继续工作——上下文管理延伸至整个会话。
